#!/usr/bin/env python3
"""
demo_opencv_two_matrices.py — OpenCV visualisation for TWO synchronised
CCD sensor heads.

CSV format from ESP32  (new firmware):
  index, <128 values head-1>, -1, <128 values head-2>

Two independent OpenCV windows are created:
  "Head 1 — Blais-Rioux"
  "Head 2 — Blais-Rioux"

Each window has the same four-panel layout as demo_opencv_br.py:
  ┌────────────────────────────────────┐
  │ HEADER — stats, k, angle, bits    │
  ├────────────────────────────────────┤
  │ ORIGINAL strip (raw ADC)          │
  ├────────────────────────────────────┤
  │ CORRECTED strip (after undist.)   │
  ├────────────────────────────────────┤
  │ BIT PANEL — undist. + bit marks   │
  └────────────────────────────────────┘

Usage
-----
  python demo_opencv_two_matrices.py --source matrix --port COM3
  python demo_opencv_two_matrices.py --source sim          # simulation
"""
...
class DualFramePacket:
    """One serial frame split into two per-head packets."""
    row_index: int
    pixels1: np.ndarray  # head 1
    pixels2: np.ndarray  # head 2
...
# ---------------------------------------------------------------------------
# Drawing constants & helpers  (identical to demo_opencv_br.py)
# ---------------------------------------------------------------------------
...
def draw_strip_2d(
    ...
    if strip_bgr.shape[1] != render_w:
        strip_bgr = cv2.resize(strip_bgr, (render_w, height),
...
def draw_header(
    ...
    if det is None:
        ...
        if det.bit_width_px > 0 and det.measured_bit_period > 0:
            period_err = (
                (det.measured_bit_period - det.bit_width_px)
                / det.bit_width_px * 100.0
            )
        lines.append((
            f"N={filter_order} thr={threshold_pct:.0f}% "
            f"T={det.measured_bit_period:.3f}px ({period_err:+.2f}%)",
            COLOR_TEXT, 0.44, 1,
        ))
        n_exp, n_got = n_bits_apriori, len(det.recovered_bit_values)
        if has_ground_truth:
            col = COLOR_OK if det.accuracy >= 99.0 else (
                COLOR_ERR if det.accuracy < 90.0 else COLOR_MUTED)
            lines.append((
                f"edges={len(det.detected_edges)} bits_apriori={n_exp} "
                f"bits_recov={n_got} rms={det.rms_edge_error:.3f}px "
            ...
            lines.append((
                f"edges={len(det.detected_edges)} bits_apriori={n_exp} "
                f"bits_recov={n_got} rms=N/A acc=N/A",
                col, 0.44, 1,
            ))
        if angle_est is None:
            ...
            lines.append((
                f"Angle: N/A full_bits={angle_est.visible_bits} "
                f"sync={angle_est.matched_windows}/{angle_est.total_windows}",
                COLOR_MUTED, 0.44, 1,
            ))
        else:
            std_s = (f"{angle_est.std_angle_deg:.5f} deg"
                     if angle_est.std_angle_deg is not None else "N/A")
            lines.append((
                f"Angle: {angle_est.mean_angle_deg:.4f} deg, std={std_s}, "
...
def draw_bit_panel(
    ...
    pts  = [(xmap(i), int(round(sig_bot - v * (sig_bot - sig_top))))
    ...
    if det is not None:
        ...
        pts2 = [(xmap(i), int(round(sig_bot - v * (sig_bot - sig_top))))
                for i, v in enumerate(vignette_norm)]
        if pts2:
            cv2.polylines(canvas, [np.array(pts2, np.int32)], False,
                          (150, 20, 20), 1, cv2.LINE_AA)

    if det is None:
        cv2.rectangle(canvas, (0, top_pad), (width - 1, height - bot_pad),
                      COLOR_EDGE, 1)
        _put(canvas, "Signal (undistorted) + full-bit recovery",
             4, top_pad + 12, COLOR_MUTED, 0.38)
        return canvas
    ...
    if boundaries:
        ...
        for b in sorted(boundaries):
            if not uniq or abs(b - uniq[-1]) > 1e-6:
                uniq.append(b)
        for bx in uniq:
            cv2.line(canvas, (xmap(bx), bit_y_hi - 6),
                     (xmap(bx), height - bot_pad), COLOR_GRID, 1, cv2.LINE_AA)
    ...
    for bs, be, bv in full_cells:
        ...
        if abs(x2 - x1) >= 8:
            _put(canvas, str(bv), (x1 + x2) // 2 - 4, yy - 3,
                 color=COLOR_REC, scale=0.38)

    for edge in det.detected_edges:
        xx  = xmap(edge.position)
        col = COLOR_RISE if edge.d1_value > 0 else COLOR_FALL
        cv2.line(canvas, (xx, top_pad), (xx, height - bot_pad), col, 1,
                 cv2.LINE_AA)
...
def _detect_full_width(
    ...
    er = detect_edges(np.asarray(pixels, dtype=np.float64), br_config, vignette)
...
def compose_frame(
    ...
    if det is not None:
        ...
        _, vis_undist, _ = restore_signal_1d(
            raw_pixels.astype(np.float64), distort_k,
            output_step_px=1.0, clip_to_adc=True, as_uint16=False,
        )
    ...
    raw_natural_w = n_raw * args.pixel_width
    ...
    bp = draw_bit_panel(
        vis_undist, det, width, bit_h,
        n_pixels=n_undist, lo=lo, hi=hi,
    )
...
# ---------------------------------------------------------------------------
# Serial / simulation packet generators
# ---------------------------------------------------------------------------

EXPECTED_PIXELS = 108

def _parse_dual_csv(text: str) -> Optional[DualFramePacket]:
    try:
        ...
        try:
            sep = values.index(-1.0)
        except ValueError:
            return None

        pixels1 = np.array(values[:sep], dtype=np.float32)
        pixels2 = np.array(values[sep + 1:], dtype=np.float32)

        if pixels1.size != EXPECTED_PIXELS or pixels2.size != EXPECTED_PIXELS:
            return None
...
def dual_matrix_packets(args):
    """Yield DualFramePacket from a serial port."""
    ser = _serial_module.Serial(args.port, args.baud, timeout=1)
    time.sleep(1.0)
    try:
        while True:
            line = ser.readline()
            pkt  = _parse_dual_csv(line.decode("utf-8", errors="ignore"))
            if pkt is not None:
                yield pkt
    finally:
        ser.close()
...
def run(args) -> None:
    dk1 = args.distort_k1 if args.distort_k1 is not None else DISTORT_K_1
    dk2 = args.distort_k2 if args.distort_k2 is not None else DISTORT_K_2

    def make_br(k: float) -> BRConfig:
        return BRConfig(
    ...
    ranger1, ranger2 = AutoRange(), AutoRange()
    win1 = "Head 1 — Blais-Rioux"
    win2 = "Head 2 — Blais-Rioux"
    ...
    if args.source == "matrix":
        ...
        for dual in dual_matrix_packets(args):
            pkt1 = FramePacket(row_index=dual.row_index,
                               pixels=dual.pixels1, source_label="matrix")
            pkt2 = FramePacket(row_index=dual.row_index,
                               pixels=dual.pixels2, source_label="matrix")

            det1 = det2 = None
            est1 = est2 = None
            try:
                det1 = _detect_full_width(
                    pkt1.pixels,
                    np.zeros(args.n_bits, dtype=np.int32), np.array([]),
                    br1, VIGNETTE_1,
                )
                est1 = _estimate_angle_if_possible(det1)
            except Exception as e:
                print(f"[WARN head1] {e}")
            try:
                det2 = _detect_full_width(
                    pkt2.pixels,
                    np.zeros(args.n_bits, dtype=np.int32), np.array([]),
                    br2, VIGNETTE_2,
                )
                est2 = _estimate_angle_if_possible(det2)
            except Exception as e:
                print(f"[WARN head2] {e}")

            frame1 = compose_frame(pkt1, args, ranger1, det1, est1,
                                   args.filter_order, args.threshold, dk1)
            frame2 = compose_frame(pkt2, args, ranger2, det2, est2,
                                   args.filter_order, args.threshold, dk2)

            if args.save:
                cv2.imwrite(args.save.replace(".", "_h1."), frame1)
                cv2.imwrite(args.save.replace(".", "_h2."), frame2)
                break
    ...
    while True:
        def sim(k, seed_offset=0):
            cfg = SimulatorConfig(
                n_bits=args.n_bits, bit_width_px=args.bit_width,
                sigma_blur_px=args.blur, noise_sigma_adu=args.noise,
                vignette_strength=args.vignette, distort_coeff=k,
                n_pixels=args.n_pixels, seed=seed + seed_offset,
            )
            return simulate_ccd(cfg)

        r1 = sim(dk1, 0)
        r2 = sim(dk2, 1)

        pkt1 = FramePacket(row_index=seed, pixels=r1.adc_signal.astype(np.float32),
                           source_label="simulation")
        pkt2 = FramePacket(row_index=seed, pixels=r2.adc_signal.astype(np.float32),
                           source_label="simulation")

        det1 = det2 = None
        est1 = est2 = None
        try:
            det1 = _detect_full_width(r1.adc_signal.astype(np.float64),
                                      r1.bits, r1.true_edges, br1, VIGNETTE_1)
            est1 = _estimate_angle_if_possible(det1)
        except Exception as e:
            print(f"[WARN head1] {e}")
        try:
            det2 = _detect_full_width(r2.adc_signal.astype(np.float64),
                                      r2.bits, r2.true_edges, br2, VIGNETTE_2)
            est2 = _estimate_angle_if_possible(det2)
        except Exception as e:
            print(f"[WARN head2] {e}")
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="OpenCV Blais-Rioux visualiser — two synchronised sensor heads."
    )
    ...
    cor = p.add_argument_group("Distortion correction (per head)")
    cor.add_argument("--distort-k1", type=float, default=None,
                     help=f"k for head 1 (default: DISTORT_K_1={DISTORT_K_1})")
    cor.add_argument("--distort-k2", type=float, default=None,
                     help=f"k for head 2 (default: DISTORT_K_2={DISTORT_K_2})")
